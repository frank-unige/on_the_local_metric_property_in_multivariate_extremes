...
if(length(triangles(graph))==0){
  cat("There are no triangles in the given graph, the algorithm returns the input. ")
  return(list(mu_opt=c(),Gamma_opt=Gamma))}
else{
lm=loc_metr(Gamma,graph)  
if(lm$lm==TRUE){
  cat("Gamma already satisfies the local metric property with respect to the given graph. ")
...
grad = function(mu){
    ...
    Q_upper[edges]=Qmatrix[edges]+Amulti(mu)
    Q=t(Q_upper)[lower.tri(Q_upper,diag=FALSE)]
    Gamma=Theta2Gamma(Qvec2Theta(Q))
    grad = -tAmulti(Gamma)
    return(grad)
}
...
return(list(mu_opt=mu_opt$solution,Gamma_opt=Theta2Gamma(Qvec2Theta(Q_opt))))
}}}
